...
interface Repuesto {
  ...
  descripcion: string;
  stock: number;
  precio_unitario: string; // Se mantiene como string por la interfaz original
}
...
export default function VerCasos() {
  ...
  // --- Efecto de Carga Inicial (useEffect) ---
  ...
  // --- Funciones de Manejo de Estado y Peticiones ---

  const abrirModal = async (caso: Caso) => {
    try {
      // Uso de backticks para la template literal string
      const res = await axios.get(`http://localhost:4000/api/casos/${caso.id_caso}`);
      setCasoActivo(res.data);
    } catch (error) {
      console.error("Error cargando detalle de caso:", error);
      setCasoActivo(caso); // Si falla el detalle, al menos muestra la data inicial
    }
  ...
  const actualizarCantidad = (id: number, cantidad: number) => {
    setMaterialesSeleccionados(prev =>
      prev.map(m => m.repuesto.id_repuesto === id ? { ...m, cantidad: isNaN(cantidad) || cantidad < 1 ? 1 : cantidad } : m)
    );
  };
  ...
  const aplicarFinalizacion = async () => {
    ...
    try {
      ...
      await axios.put(`http://localhost:4000/api/casos/cerrar/${casoActivo.id_caso}`, {
        id_empleado: 2, // ID del usuario logueado (Ajustar según la lógica de autenticación)
        detalles,
        complicacion,
      ...
      alert("✅ Caso cerrado y actualizado correctamente.");
      cerrarModal();
    } catch (error) {
  ...
  const asignarTecnico = async () => {
    ...
    if (!casoActivo.id_caso) {
      alert("Error: no hay caso seleccionado o el ID del caso es inválido.");
      console.error("Caso activo inválido:", casoActivo);
      return;
    }

    console.log("Intentando asignar técnico", tecnicoSeleccionado, "al caso", casoActivo.id_caso);
    ...
    try {
      ...
      // Actualizamos la información del caso
      // Uso de backticks para la template literal string
      const casoActualizadoRes = await axios.get(
        `http://localhost:4000/api/casos/${casoActivo.id_caso}` // Ajustada la URL para traer el detalle actualizado
      );

      // Usar directamente el objeto de caso que trae la respuesta si tiene la estructura esperada
      const casoActualizado: Caso = casoActualizadoRes.data; 
      
      setCasoActivo(casoActualizado);
      setCasos(prev =>
        prev.map(c => c.id_caso === casoActualizado.id_caso ? casoActualizado : c)
      );
      setTecnicoSeleccionado("");
    } catch (error: any) {
      console.error("Error asignando técnico:", error);
      alert("❌ No se pudo asignar el técnico. Revisa la consola para más detalles.");
    }
  };
        ...
        <tbody>
          {casos.map((caso) => (
            <tr
              key={caso.id_caso}
              style={{
                backgroundColor: caso.estado_actual === "Finalizado" ? "#e6f4ea" : "#f9f9f9",
              }}
            >
                    ...
                    {materialesSeleccionados.map(m => (
                      <div key={m.repuesto.id_repuesto} style={{ display: "flex", alignItems: "center", marginBottom: "0.5rem" }}>
                        ...
                        <input
                          type="number"
                          min={1}
                          max={m.repuesto.stock} // Limitar a la cantidad de stock
                          value={m.cantidad}
