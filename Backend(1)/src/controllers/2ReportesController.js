...
/**
 * Controlador que ejecuta el procedimiento PL/SQL PKG_ENCUESTA_SATISFACCION.GENERAR_REPORTE
 */
export const generarReporteEncuesta = async (req, res) => {
  ...
  if (!tipo_reporte || !fecha_inicio || !fecha_fin) {
    return res.status(400).json({
      ok: false,
      msg: "Faltan parámetros requeridos (tipo_reporte, fecha_inicio, fecha_fin)",
    });
  }

  try {
    // 🔹 Asegurar que el pool Oracle esté inicializado
    await initOraclePool();
    const conn = await oracledb.getConnection();

    // 🧠 Ejecutar el procedimiento del paquete
    const result = await conn.execute(
      `
      BEGIN
        PKG_ENCUESTA_SATISFACCION.GENERAR_REPORTE(
          :p_tipo_reporte,
    ...
    // 📦 Devolver respuesta uniforme
    return res.json({ ok: true, data });
  } catch (err) {
    console.error("❌ Error en generarReporteEncuesta:", err);
    return res.status(500).json({
      ok: false,
      msg: "Error al generar reporte de encuestas",
      error: err.message,
    });
  }
};
